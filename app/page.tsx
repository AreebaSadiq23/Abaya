...
const featuredProducts = [
  { id: 1, name: "Classic Black Abaya", price: 89, image: "/assets/images/1.jfif", description: "Timeless elegance for every occasion." },
  { id: 2, name: "Embroidered Taupe", price: 129, image: "/assets/images/2.jfif", description: "Subtle details, sophisticated charm." },
  { id: 3, name: "Minimalist Beige", price: 99, image: "/assets/images/3.jfif", description: "Effortless style, refined quality." },
];
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section className="py-10 px-5 md:py-16 md:px-10 xl:px-32">
        <div className="grid grid-cols-2 gap-2 md:gap-4 h-[40vh] md:h-[50vh] xl:h-[60vh]">
          <div className="col-span-2 row-span-1 md:row-span-2 relative bg-muted">
            <Image src="/assets/images/5.jfif" alt="Lifestyle" fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" quality={100} />
          </div>
          <div className="relative bg-muted">
            <Image src="/assets/images/6.jfif" alt="Lifestyle" fill className="object-cover" sizes="(max-width: 768px) 50vw, 25vw" quality={100} />
          </div>
          <div className="relative bg-muted">
            <Image src="/assets/images/7.jfif" alt="Lifestyle" fill className="object-cover" sizes="(max-width: 768px) 50vw, 25vw" quality={100} />
          </div>
        </div>
      </section>
      ...
      <section className="py-12 md:py-20 xl:py-32 px-5 md:px-10 xl:px-32">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 xl:gap-8">
          {['Evening Elegance', 'Daytime Chic', 'Minimalist Essentials'].map((guide, i) => (
            <Link key={guide} href="/shop">
              <div className="group relative aspect-[3/4] bg-muted overflow-hidden">
                <Image 
                  src={`/assets/images/${i + 9}.jfif`} 
                  alt={guide} 
                  fill 
      ...
      <section className="py-12 md:py-20 xl:py-32 bg-background">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 xl:gap-16 px-5 md:px-10 xl:px-32">
          {featuredProducts.map((product, index) => (
            <Link key={product.id} href={`/product/${product.id}`} className={`group space-y-4 md:space-y-6 xl:space-y-8 ${index === 1 ? 'lg:mt-16 xl:mt-32' : ''}`}>
